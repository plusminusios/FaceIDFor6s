...
static void FIDLoadPrefs(void) {
    ...
    gFaceEnrolled   = d[@"faceEnrolled"]  ? [d[@"faceEnrolled"] boolValue]     : NO;
}
...
- (void)runWithCompletion:(void(^)(BOOL))cb {
    ...
    [out setSampleBufferDelegate:self
                           queue:dispatch_queue_create("fid.scan", DISPATCH_QUEUE_SERIAL)];
...
- (instancetype)initWithFrame:(CGRect)f {
    ...
    CGFloat w = MIN(f.size.width, f.size.height) * 0.52, h = w * 1.28;
    ...
    brd.strokeColor = [UIColor colorWithRed:0.1 green:0.6 blue:1 alpha:0.9].CGColor;
    brd.fillColor = UIColor.clearColor.CGColor;
    brd.lineWidth = 3;
    [self.layer addSublayer:brd];

    _line = [CALayer layer];
    _line.frame = CGRectMake(_oval.origin.x+6, _oval.origin.y, _oval.size.width-12, 2);
    ...
    title.font = [UIFont systemFontOfSize:13 weight:UIFontWeightLight];
    title.textAlignment = NSTextAlignmentCenter;
    title.frame = CGRectMake(0, _oval.origin.y - 32, f.size.width, 20);
    ...
    _lbl.frame = CGRectMake(16, CGRectGetMaxY(_oval)+22, f.size.width-32, 22);
...
- (void)showOK:(BOOL)ok {
    ...
    _lbl.text = ok ? @"✓  Готово" : @"✕  Не удалось";
    _lbl.textColor = ok
        ...
        [UIView animateWithDuration:0.2 animations:^{ ov.alpha = 1; }
            ...
            [[FIDScanner shared] runWithCompletion:^(BOOL ok) {
                ...
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW,
                    (int64_t)(0.65*NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
...
- (void)_evaluateBiometricAuthentication {
    FIDLoadPrefs();
    if (!gEnabled || !gFaceEnrolled) { %orig; return; }
